...
export default function StewardLoginSection() {
  ...
  const setSessionCookie = useCallback(async (token: string, refreshToken?: string | null) => {
    ...
    // Routing through the same-origin Pages Functions proxy would set the
    // Set-Cookie on www.elizacloud.ai, but every subsequent apiFetch call
    // (e.g. /auth/cli-session/<id>/complete) goes cross-origin to
    // api.elizacloud.ai and the host-only cookie does not flow with it,
    // which deadlocks the CLI login spinner at "Generating API Key".
