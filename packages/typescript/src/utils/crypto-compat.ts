...
export function createCipheriv(
	...
	return {
		update(
			...
			const fullBlockLength = pending.length - (pending.length % AES_BLOCK_SIZE);
...
export function createDecipheriv(
	...
	return {
		...
		final(encoding: string): string {
			if (
				pending.length === 0 ||
				pending.length % AES_BLOCK_SIZE !== 0
			) {
				throw new Error("Invalid ciphertext length for AES-CBC payload.");
			}
...
export function decryptAes256Gcm(
	...
	const combined = concatBytes(Uint8Array.from(ciphertext), Uint8Array.from(tag));
