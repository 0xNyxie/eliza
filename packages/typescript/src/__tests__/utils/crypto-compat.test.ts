...
import { createCipheriv as createNodeCipheriv, createDecipheriv as createNodeDecipheriv, createHash as createNodeHash } from "node:crypto";
...
describe("crypto-compat", () => {
	...
	describe("createCipheriv / createDecipheriv (Node.js synchronous)", () => {
		...
		it("should match node crypto for split CBC updates", async () => {
			...
			expectedEncrypted += nodeCipher.update(plaintext.slice(19), "utf8", "hex");
