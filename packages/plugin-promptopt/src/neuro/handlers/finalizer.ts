  ...
  for (const trace of traces) {
    try {
          ...
          runtime.logger.debug(
            {
              src: "plugin-neuro",
              error:
                signalCtxErr instanceof Error
                  ? signalCtxErr.message
                  : String(signalCtxErr),
            },
      ...
      if (!autoOptScheduled.has(autoKey)) {
        ...
        // Cross-event deduplication is handled by runLocks in auto-optimizer.ts
        autoOptPromises.push(
          maybeRunAutoPromptOptimization(runtime, optDir, trace).catch((err) => {
            runtime.logger.warn(
              {
                src: "plugin-neuro",
                runId,
